    ...
    >
      <div className={styles.groupCardHeader}>
        <div className={styles.groupCheckbox}>
          <Checkbox
            checked={selected}
            onChange={(_, data) => onSelect(group.id, data.checked === true)}
          />
        </div>
        ...
        <div className={styles.groupInfo}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '2px' }}>
            {group.category && (
              <Badge size="extra-small" color="brand">
                {group.category}
              </Badge>
            )}
            <Badge size="extra-small" appearance="outline">
              {group.variables.length}
            </Badge>
          </div>
          <Text size={300} weight="semibold" style={{ // 减小字体
            display: 'block',
            marginBottom: '1px', // 进一步减小间距
      ...
      <div className={styles.groupCardContent}>
        <div style={{ maxHeight: '40px', overflow: 'hidden' }}> {/* 进一步减小最大高度 */}
          {group.variables.slice(0, 2).map((variable, index) => ( // 只显示2个变量
            <div key={index} className={styles.variableItem}>
              <strong style={{ color: tokens.colorNeutralForeground2 }}>{variable.key}</strong>: {variable.value}
            </div>
          ))}
